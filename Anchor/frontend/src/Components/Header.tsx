...
import { Menu, X, Anchor as AnchorIcon } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-10">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
              className="text-sm font-medium text-gray-400 hover:text-[#39ff14] transition-all relative group cursor-pointer"
            >
              {item.name}
        ...
        <div className="hidden lg:flex items-center space-x-6">
          <button
            ...
            className="text-sm font-medium text-white hover:text-[#39ff14] transition-colors"
          >
            Sign In
          </button>
          <button
            onClick={() => navigate('/register')}
            className="px-6 py-2.5 rounded-full bg-[#39ff14] text-black font-bold text-sm hover:bg-[#39ff14]/90 hover:shadow-[0_0_20px_rgba(57,255,20,0.4)] transition-all transform hover:-translate-y-0.5 active:scale-95"
          >
            Get Started
          </button>
        </div>
        ...
        <div className="flex flex-col h-full pt-24 px-8 space-y-8">
          {navItems.map((item) => (
            <button
              ...
              className="text-3xl font-bold text-white hover:text-[#39ff14] transition-colors text-left"
            >
              {item.name}
            </button>
          ))}
          <div className="pt-8 space-y-6">
            <button
              ...
              className="block w-full text-left text-2xl font-bold text-white hover:text-[#39ff14]"
            >
              Sign In
            </button>
            <button
              onClick={() => handleNavClick('/register')}
              className="w-full py-4 rounded-2xl bg-[#39ff14] text-black font-black text-xl"
            >
              Get Started
            </button>
          </div>
